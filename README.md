...
| `extensions.nginx.email: admin@example.com` | ACME via certbot sidecar (ports 80 + 443) — nginx bootstraps a throwaway self-signed cert per domain so it can start before certbot has issued the real one, then reloads every 6h to pick it up; certbot retries a failed first issuance with capped exponential backoff (5min → 60min, tuned to stay under Let's Encrypt's 5-authorization-failures-per-identifier-per-hour limit) before settling into a 12h renew loop |
